...
def fusion_meteo_sol_continental(
    ...
    max_distance_deg=0.3  # 👈 Change ici pour agrandir le rayon (~30 km)
):
    print("🔁 Lancement de la fusion météo-sol continentale...")
    ...
    for file in os.listdir(folder_meteo):
        ...
        try:
            ...
            if not {'Longitude', 'Latitude', 'Country', 'DATE'}.issubset(df_met.columns):
                print(f"❌ {file} : colonnes manquantes, ignoré")
                continue
            ...
            if df_met.empty:
                print(f"❌ {file} : fichier météo vide")
                continue

            df_sol_match = match_country(df_sol_all, meteo_name)
            if df_sol_match.empty:
                print(f"⚠️ {file} → Aucun point sol trouvé pour : {meteo_name}")
                continue

            # 🧠 Jointure par distance
    ...
    if all_dfs:
        ...
        print(f"📊 Points totaux fusionnés : {len(df_final)} | Pays inclus : {df_final['Country'].nunique()}")
        return df_final
    else:
        print("❌ Aucun fichier météo n’a pu être fusionné.")
        return pd.DataFrame()
